...
function ClassifyContent() {
  ...
  const loadItems = useCallback(() => {
    fetch(`/api/classify?${queryString}`)
      .then((r) => r.json())
      .then((data) => { setItems(data.items); setCurrentIndex(0); setClassified([]); })
  ...
  const classify = useCallback((classification: "professional" | "personal") => {
    if (!current) return;
    setSwipeDir(classification === "personal" ? "left" : "right");
    setTimeout(() => {
      setClassified((prev) => [...prev, { id: current.id, type: current.type, classification, name: current.name }]);
  ...
  const saveAll = async () => {
    ...
    setSaving(true);
    try {
      await fetch("/api/classify", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ classifications: classified }) });
      loadItems();
    } catch (e) { alert("Fout bij opslaan: " + (e as Error).message); }
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="mb-6">
        <div className="flex justify-between text-sm text-gray-500 mb-2"><span>{classified.length} geclassificeerd</span><span>{remaining} resterend</span></div>
        <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden"><div className="h-full bg-blue-500 rounded-full transition-all" style={{ width: `${items.length > 0 ? (classified.length / items.length) * 100 : 0}%` }} /></div>
      </div>
        ...
        <div className="text-center py-16">
          {classified.length > 0 ? (
            <>
              ...
              <p className="text-xl font-semibold text-gray-900 mb-2">{classified.length} items geclassificeerd</p>
              <button onClick={saveAll} disabled={saving} className="mt-4 px-6 py-3 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 disabled:opacity-50 transition-colors">
                {saving ? "Opslaan..." : "Alles opslaan"}
              </button>
            </>
          ) : (
            <><p className="text-xl font-semibold text-gray-900">Alles is geclassificeerd!</p><p className="text-gray-500 mt-2">Geen items meer om te classificeren dit kwartaal.</p></>
          )}
        </div>
      ) : (
        <>
          ...
          <div className="flex justify-center gap-8">
            <button onClick={() => classify("personal")} className="flex flex-col items-center gap-2 group">
          ...
          {classified.length > 0 && (
            <div className="mt-8 text-center">
              <button onClick={saveAll} disabled={saving} className="px-6 py-2 bg-gray-900 text-white rounded-lg text-sm font-medium hover:bg-gray-800 disabled:opacity-50 transition-colors">
...
export default function ClassifyPage() {
  return <Suspense fallback={<div className="flex items-center justify-center h-64 text-gray-500">Laden...</div>}><ClassifyContent /></Suspense>;
}
